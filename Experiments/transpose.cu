 ...
 */
	...
	ispc::gridDim grid_dim;
	grid_dim.gridDim_x = N / K;
	grid_dim.gridDim_y = N / K;
	grid_dim.gridDim_z = 1;
	ispc::blockDim block_dim;
	block_dim.blockDim_x = K;
	block_dim.blockDim_y = K;
	block_dim.blockDim_z = 1;
	transpose_parallel_per_element_ispc(grid_dim, block_dim, in, out);
	printf("transpose_parallel_per_element_ispc: %g ms.\nVerifying transpose...%s\n",
		   timer.Elapsed(), compare_matrices(out, gold) ? "Failed" : "Success");

	memset(out, 0, numbytes);

	transpose_parallel_per_element_ispc_task(grid_dim, block_dim, in, out);
	printf("transpose_parallel_per_element_ispc_task: %g ms.\nVerifying transpose...%s\n",
		   timer.Elapsed(), compare_matrices(out, gold) ? "Failed" : "Success");
	/*
	timer.Start();
